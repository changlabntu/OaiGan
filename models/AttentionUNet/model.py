...
class AttU_Net(nn.Module):
    def __init__(self, n_channels=3, n_classes=1, scale_factor=1, use_attention=(True, True, True, True)):
    ...
    def forward(self, x):
        ...
        d3 = self.Up3(d4)
        if self.use_attention[2]:
            x2, psi3 = self.Att3(g=d3, x=x2)
            psi_all.append(psi3)
        ...
        d2 = self.Up2(d3)
        if self.use_attention[3]:
            x1, psi2 = self.Att2(g=d2, x=x1)
            psi_all.append(psi2)
...
if __name__ == '__main__':
    model = AttU_Net(n_channels=3, n_classes=1, scale_factor=1, use_attention=(False, False, False, True))
